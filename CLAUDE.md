...
Each domain maintains its own CLAUDE.md with workflows and context; this file is the ROUTER — global policies + pointers, not content.
...
**Domain Context Loading:** when a domain trigger is invoked, FIRST read the domain's CLAUDE.md, then execute its "Check [Domain]" workflow. **Do not execute Check workflows for domains whose `state` is not `active`** — surface the state (complete/dormant/killed) instead.

## Subagents

Definitions in `.claude/agents/`. Delegate to protect the main context window — searching, research, and review belong in subagents, not inline.
