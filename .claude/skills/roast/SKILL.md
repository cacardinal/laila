---
name: roast
description: Adversarial idea/plan critic. Use when Alex says "/roast", "roast this idea/plan", "pressure-test" or "stress-test this", "poke holes in this", "what am I missing", "convene the council", or wants a brutal second opinion BEFORE committing >4 hours or >$500 to any plan, decision, strategy, or venture. Spins up a 5-persona council that attacks the idea from every angle, then a Judge returns one GO / RESHAPE / KILL verdict with the cheapest test to de-risk it.
argument-hint: "[the plan or idea to roast]"
---
...
If `$ARGUMENTS` contains the idea, start there. Then ask Alex a tight set of clarifying questions so the council has real context to work with. Ask only what hasn't already been provided. Keep it to 3-4 questions max, in one batch:
...
3. **The edge** — relevant skills, audience, or assets Alex already has.
4. **Constraints** — budget, timeline, how fast first dollar is needed.

If Alex says "just run it" or gives you enough already, skip the questions and proceed. Don't over-interrogate. One round, then convene the council.
...
Each council member must return: a one-line stance, their 3-5 sharpest points, the single most important thing Alex must hear, and a 1-10 score on their own dimension (1 = walk away, 10 = no-brainer).
...
Before writing the verdict, fold in two lenses yourself:

- **Alex's own history** — read `knowledge/tacit/bottlenecks.md` and grep `knowledge/decisions/` for prior similar attempts. How did they go? Cite specifics. Flag sunk-cost reasoning ("we've already built X so...") and shiny-object drift (a new direction crowding out stated priorities in `state/strategy.md`) explicitly.
- **Economics** — rough pricing, realistic time-to-first-dollar, and whether Alex can actually ship this fast given the edge they described.
...
**History check:** [what Alex's own bottlenecks/decisions say about this pattern]

**Money read:** [rough price, time-to-first-dollar, can they ship fast]

**The cheapest 48-hour test:** [the smallest, fastest thing Alex can do
to validate the riskiest assumption BEFORE building anything]

**Kill criteria:** Kill this if [concrete, checkable condition] by [DATE].
