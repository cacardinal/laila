...
5. **Check Email — ALL configured accounts.** Missing any account is a bug:
   - Personal inbox (e.g. a `gmail-personal` MCP server) — run each active domain's email filters (defined in the domain AGENTS.md files): career, health, household, content, acme
